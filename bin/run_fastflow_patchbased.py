...
def save_segmentation_visualizations(results_path, dataset, anomaly_maps, image_scores, max_images=200):
    ...
    for idx in range(num_images):
        try:
            ...
            # Get anomaly map
            anomaly_map = anomaly_maps[idx].squeeze().numpy() if hasattr(anomaly_maps[idx], 'numpy') else anomaly_maps[idx]
            ...
            axes[1].imshow(denorm_image, alpha=0.3)
            axes[1].set_title('Ground Truth')
            axes[1].axis('off')
            
            # Anomaly heatmap
            axes[2].imshow(anomaly_map, cmap='jet', alpha=0.7)
            axes[2].imshow(denorm_image, alpha=0.3)
            ...
            plt.savefig(os.path.join(segmentation_path, save_name), dpi=150, bbox_inches='tight')
            plt.close()
            
        except Exception as e:
            LOGGER.warning(f"Could not save visualization for image {idx}: {e}")
            continue
...
@click.option("--patchsize", type=int, default=3, help="Size of patches for patch-based processing")
@click.option("--patchstride", type=int, default=1, help="Stride for patch extraction")
def fastflow_patch(backbone, flow_steps, input_size, conv3x3_only, hidden_ratio, use_class_specific, patchsize, patchstride):
    def get_fastflow_patch():
        ...
        model = FastFlow(
            ...
            enable_patch_processing=True,  # Enable patch-based processing
            patchsize=patchsize,
            patchstride=patchstride,
        )
        return model
    return ("get_fastflow_patch", get_fastflow_patch)
...
def run(methods, results_path, gpu, seed, log_group, log_project, save_model, save_segmentation_images, eval_only, checkpoint):
    ...
    # Get backbone name from get_fastflow_patch closure
    backbone_name = "resnet18"  # default
    if "get_fastflow_patch" in methods:
        closure_vars = inspect.getclosurevars(methods["get_fastflow_patch"])
    ...
    # Check if class-specific parameters should be used
    use_class_params = any('use_class_specific' in str(result) for result in methods.items())
    if category in param_set and use_class_params:
        ...
        model = methods["get_fastflow_patch"]().to(device)
        LOGGER.info(f"Using default params: lr={learning_rate}, epochs={num_epochs}")
    
    LOGGER.info(f"FastFlow model created with patch-based processing enabled")
    LOGGER.info(f"Patch size: {model.patch_maker.patchsize}, Patch stride: {model.patch_maker.stride}")
    ...
    if not eval_only:
        LOGGER.info("Starting patch-based FastFlow training...")
        # Training loop
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch % EVAL_INTERVAL == 0:
                # Quick evaluation using patch-based processing
                model.eval()
                eval_scores = []
                eval_labels = []
                with torch.no_grad():
                    for batch in dataloaders["testing"]:
                        ...
                        # Use the model's predict_dataloader method for consistency
                        anomaly_maps = model.predict(batch)
                        
                        # Compute image-level scores using patch aggregation
                        image_scores = torch.max(anomaly_maps.view(anomaly_maps.shape[0], -1), dim=1)[0]
                        
                        # Get labels
                        labels = (torch.max(batch["mask"].view(batch["mask"].shape[0], -1), dim=1)[0] > 0.5).float()
                        
                        eval_scores.extend(image_scores.cpu().numpy())
                        eval_labels.extend(labels.cpu().numpy())
                
                if len(set(eval_labels)) > 1:  # Check if both classes are present
                    auroc = roc_auc_score(eval_labels, eval_scores)
                    LOGGER.info(f"[Epoch {epoch}] Image-level AUROC (patch-based): {auroc:.4f}")
                else:
                    LOGGER.info(f"[Epoch {epoch}] AUROC: N/A (only one class present)")
                    
            if epoch % CHECKPOINT_INTERVAL == 0 and save_model:
                torch.save(model.state_dict(), os.path.join(results_path, f"fastflow_patch_{log_group}_epoch{epoch}.pth"))
        
        # Save final model
        if save_model:
            torch.save(model.state_dict(), os.path.join(results_path, f"fastflow_patch_{log_group}_final.pth"))

    # Final Evaluation using patch-based processing similar to PatchCore
    LOGGER.info("Starting comprehensive patch-based evaluation...")
    model.eval()
    
    # Use the model's predict_dataloader method for consistency with PatchCore
    scores, masks, labels_gt, masks_gt = model.predict_dataloader(dataloaders["testing"])
    ...
    binary_masks = (np.array(masks_gt) > 0.5).astype(float)
    
    # Compute comprehensive metrics
    # 1. Instance-level AUROC (image-level anomaly detection)
    try:
        if len(set(labels_gt)) > 1:
            instance_auroc = roc_auc_score(labels_gt, scores)
        else:
            instance_auroc = 0.5
            LOGGER.warning("Only one class present in instance labels - using default AUROC of 0.5")
    except Exception as e:
        instance_auroc = 0.5
        LOGGER.warning(f"Error computing instance AUROC: {e} - using default of 0.5")
    
    # 2. Full pixel-level AUROC (all images)
    try:
        pixel_scores_flat = masks.flatten()
        pixel_labels_flat = binary_masks.flatten()
        if len(set(pixel_labels_flat)) > 1:
            full_pixel_auroc = roc_auc_score(pixel_labels_flat, pixel_scores_flat)
        else:
            full_pixel_auroc = 0.5
            LOGGER.warning("Only one class present in pixel labels - using default AUROC of 0.5")
    except Exception as e:
        full_pixel_auroc = 0.5
        LOGGER.warning(f"Error computing full pixel AUROC: {e} - using default of 0.5")
    
    # 3. Anomaly pixel-level AUROC (only anomalous images)
    try:
        anomalous_indices = np.where(labels_gt > 0.5)[0]
        if len(anomalous_indices) > 0:
            anomaly_masks = binary_masks[anomalous_indices]
            anomaly_maps = masks[anomalous_indices]
            if len(set(anomaly_masks.flatten())) > 1:
                anomaly_pixel_auroc = roc_auc_score(anomaly_masks.flatten(), anomaly_maps.flatten())
            else:
                anomaly_pixel_auroc = 0.5
                LOGGER.warning("Only one class present in anomaly pixel labels - using default AUROC of 0.5")
        else:
            anomaly_pixel_auroc = 0.5
            LOGGER.info("No anomalous images found - anomaly pixel AUROC set to 0.5")
    except Exception as e:
        anomaly_pixel_auroc = 0.5
        LOGGER.warning(f"Error computing anomaly pixel AUROC: {e} - using default of 0.5")
    
    # Compute sPRO (soft PRO)
    spro = compute_spro(masks, binary_masks)

    # Log results
    LOGGER.info("=== FastFlow Patch-Based Evaluation Results ===")
    ...
    # Save results to CSV
    dataset_name = f"fastflow_patch_{dataloaders['testing'].dataset.classnames_to_use[0]}"
    results = {
    ...
    # Save to CSV file
    csv_path = os.path.join(results_path, "fastflow_patch_results.csv")
    ...
    if save_segmentation_images:
        save_segmentation_visualizations(results_path, dataloaders["testing"].dataset, masks, scores)
